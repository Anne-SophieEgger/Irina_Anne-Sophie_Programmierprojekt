
#pygame hangman

import pygame, sys, random
from time import sleep
from pygame.locals import *
from timeit import default_timer as timer

fps = 30
pygame.init()
width = 800
height = 600

black = (0,0,0)
white = (255,255,255)
lightblue = (99, 122, 161)
darkblue = (30, 90, 150)
lightgrey = (192, 192, 192)

textBoxSpace = 5
textBoxNumber = 0

def button(word,x,y,w,h,ic,ac,action=None):
    mouse = pygame.mouse.get_pos()
    click = pygame.mouse.get_pressed()

    if x+w > mouse[0] > x and y+h > mouse[1] > y:
        pygame.draw.rect(screen,ac,(x,y,w,h))
        if click[0] == 1 and action != None:
            action()
    else:
        pygame.draw.rect(screen,ic,(x,y,w,h))

    buttonText = pygame.font.Font("freesansbold.ttf",20)
    buttonTextSurf = buttonText.render(word, True, white)
    buttonTextRect = buttonTextSurf.get_rect()
    buttonTextRect.center = ((x+(w/2)), (y+(h/2)))
    screen.blit(buttonTextSurf, buttonTextRect)

def endGame():
    global textBoxSpace, textBoxNumber, end, start
    end = timer()
    print("Benötigte Zeit: ",end - start)
    timeTaken = (end - start)
    textBoxSpace = 5
    textBoxNumber = 0
    message = "Zeit:" + str(round(timeTaken)) + "s"
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
                
        button("Ja",(width/2)-50,420,100,50,darkblue,lightblue,quitGame)
        button("Nein",(width/2)-50,500,100,50,darkblue,lightblue,hangman)
        
        largeText = pygame.font.SysFont("comicsansms",115)
        TextSurf = largeText.render("Spiel beenden?",True,darkblue)
        TextRect = TextSurf.get_rect()
        TextRect.center = (width / 2, height / 2)
        screen.blit(TextSurf, TextRect)

        textSurf = largeText.render(message,True,darkblue)
        textRect = textSurf.get_rect()
        textRect.center = (width/2,200)
        screen.blit(textSurf, textRect)
        
        pygame.display.update()
        clock.tick(fps)
        
def quitGame():
    pygame.quit()
    sys.exit()

def unpause():
    global pause
    pause = False

def pause():
    largeText = pygame.font.SysFont("comicsansms",115)
    TextSurf = largeText.render("Paused",True,black)
    TextRect = TextSurf.get_rect()
    TextRect.center = (width / 2, height / 2)
    screen.blit(TextSurf, TextRect)

    while pause:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
                
        screen.fill(white)
        

        button("Weiterfahren",150,450,100,50,darkblue,lightblue,unpause)
        button("Verlassen",550,450,100,50,darkblue,lightblue,quitgame)

        pygame.display.update()
        clock.tick(fps)

def textObjects(text, font):
    textSurface = font.render(text, True, black)
    return textSurface, textSurface.get_rect()
    
def main():
    global clock, screen, play
    play = True
    clock = pygame.time.Clock()
    screen = pygame.display.set_mode((width, height))
    pygame.display.set_caption("Hangman!")

    while True:
        hangman()

def placeLetter(letter):
    global pick, pickSplit
    space = 10
    wordSpace = 0
    while wordSpace < len(pick):
        text = pygame.font.Font('freesansbold.ttf',40)
        if letter in pickSplit[wordSpace]:
            textSurf = text.render(letter,True,black)
            textRect = textSurf.get_rect()
            textRect.center = (((150)+space),(200))
            screen.blit(textSurf, textRect)
        wordSpace += 1
        space += 60

    pygame.display.update()
    clock.tick(fps)
        
def textBoxLetter(letter):
    global textBoxSpace, textBoxNumber
    if textBoxNumber <= 5:
        text = pygame.font.Font("freesansbold.ttf",40)
        textSurf = text.render(letter,True,black)
        textRect = textSurf.get_rect()
        textRect.center = (((105)+textBoxSpace),(350))
        screen.blit(textSurf, textRect)

    elif textBoxNumber <= 10:
        text = pygame.font.Font("freesansbold.ttf",40)
        textSurf = text.render(letter,True,black)
        textRect = textSurf.get_rect()
        textRect.center = (((105)+textBoxSpace),(400))
        screen.blit(textSurf, textRect)

    elif textBoxNumber <= 15:
        text = pygame.font.Font("freesansbold.ttf",40)
        textSurf = text.render(letter,True,black)
        textRect = textSurf.get_rect()
        textRect.center = (((105)+textBoxSpace),(450))
        screen.blit(textSurf, textRect)

    elif textBoxNumber <= 20:
        text = pygame.font.Font("freesansbold.ttf",40)
        textSurf = text.render(letter,True,black)
        textRect = textSurf.get_rect()
        textRect.center = (((105)+textBoxSpace),(500))
        screen.blit(textSurf, textRect)  
        
    pygame.display.update()
    clock.tick(fps)

def hangman():
    global textBoxSpace, textBoxNumber
    textBoxSpace = 5
    textBoxNumber = 0
    while play == True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()

        screen.fill(white)
        space = 10
        textBoxSpace = 5
        
        text = pygame.font.Font("freesansbold.ttf",20)
        textSurf = text.render("Wähle eine Kategorie",True,black)
        textRect = textSurf.get_rect()
        textRect.center = ((width/2),(height/2))
        screen.blit(textSurf, textRect)

        button("Tiere",150,450,150,100,darkblue,lightblue,Animals)
        button("Fahrzeuge",550,450,150,100,darkblue,lightblue,Vehicles)
        button("Essen",150,50,150,100,darkblue,lightblue,Foods)
        button("Sport",550,50,150,100,darkblue,lightblue,Sports)
                    
        pygame.display.update()
        clock.tick(fps)

def hangmanGame(catagory,title):
    global pause, pick, pickSplit, textBoxSpace, textBoxNumber, start
    start = timer()
    chances = 16
    pick = random.choice(catagory)
    pickSplit = [pick[i:i+1] for i in range(0, len(pick), 1)]
    
    screen.fill(white)
    
    wordSpace = 0
    space = 10
    while wordSpace < len(pick):
        text = pygame.font.Font("freesansbold.ttf",40)
        textSurf1 = text.render("_",True,black)
        textRect1 = textSurf1.get_rect()
        textRect1.center = (((150)+space),(200))
        screen.blit(textSurf1, textRect1)
        space = space + 60
        wordSpace += 1
            
    guesses = ''
    gamePlay = True
    while gamePlay == True:
        guessLett = ''

        if textBoxNumber == 5:
            textBoxSpace = 5
        if textBoxNumber == 10:
            textBoxSpace = 5
        if textBoxNumber == 15:
            textBoxSpace = 5

        pygame.draw.rect(screen, white, [550,20,200,20])
        text = pygame.font.Font("freesansbold.ttf",20)
        textSurf = text.render(("Chances: %s" % chances),False,black)
        textRect = textSurf.get_rect()
        textRect.topright = (700,20)
        screen.blit(textSurf, textRect)

        textTitle = pygame.font.Font("freesansbold.ttf",40)
        textTitleSurf = textTitle.render(title,True,black)
        textTitleRect = textTitleSurf.get_rect()
        textTitleRect.center = ((width/2),50)
        screen.blit(textTitleSurf, textTitleRect)

        pygame.draw.rect(screen, black, [100,300,250,250],2)

        if chances == 19:
            pygame.draw.rect(screen,black,[450,550,100,10])
        elif chances == 18:
            pygame.draw.rect(screen,black,[550,550,100,10])
        elif chances == 17:
            pygame.draw.rect(screen,black,[650,550,100,10])
        elif chances == 16:
            pygame.draw.rect(screen,black,[500,450,10,100])
        elif chances == 15:
            pygame.draw.rect(screen,black,[500,350,10,100])
        elif chances == 14:
            pygame.draw.rect(screen,black,[500,250,10,100])
        elif chances == 13:
            pygame.draw.rect(screen,black,[500,250,150,10])
        elif chances == 12:
            pygame.draw.rect(screen,black,[600,250,100,10])
        elif chances == 11:
            pygame.draw.rect(screen,black,[600,250,10,50])
        elif chances == 10:
            pygame.draw.line(screen,black,[505,505],[550,550],10)
        elif chances == 9:
            pygame.draw.line(screen,black,[550,250],[505,295],10)
        elif chances == 8:
            pygame.draw.line(screen,black,[505,505],[460,550],10)
        elif chances == 7:
            pygame.draw.circle(screen,black,[605,325],30)
        elif chances == 6:
            pygame.draw.rect(screen,black,[600,350,10,60])
        elif chances == 5:
            pygame.draw.rect(screen,black,[600,410,10,60])
        elif chances == 4:
            pygame.draw.line(screen,black,[605,375],[550,395],10)
        elif chances == 3:
            pygame.draw.line(screen,black,[605,375],[650,395],10)
        elif chances == 2:
            pygame.draw.line(screen,black,[605,465],[550,485],10)
        elif chances == 1:
            pygame.draw.line(screen,black,[605,465],[650,485],10)

        button("Zurück",50,50,100,50,darkblue,lightblue,hangman)
        
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
        
            if event.type == pygame.KEYDOWN:
                failed = 0
                print("Failed",failed)
                print("Versuche", chances)
                
                if event.key == pygame.K_SPACE:
                    pause()
                    
                if event.key == pygame.K_ESCAPE:
                    gamePlay = False
                    
                if event.key == pygame.K_a:
#Buchstabe a
                    guessLett = guessLett + 'a'
                    guesses += guessLett
                    print("Buchstabe a gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('a')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('a')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_b:
#Buchstabe b
                    guessLett = guessLett + 'b'
                    guesses += guessLett
                    print("Buchstabe b gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('b')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('b')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_c:
#Buchstabe c
                    guessLett = guessLett + 'c'
                    guesses += guessLett
                    print("Buchstabe c gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('c')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('c')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_d:
#Buchstabe d
                    guessLett = guessLett + 'd'
                    guesses += guessLett
                    print("Buchstabe d gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('d')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('d')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_e:
#Buchstabe e
                    guessLett = guessLett + 'e'
                    guesses += guessLett
                    print("Buchstabe e gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('e')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('e')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_f:
#Buchstabe f
                    guessLett = guessLett + 'f'
                    guesses += guessLett
                    print("Buchstabe f gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('f')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('f')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_g:
#Buchstabe g
                    guessLett = guessLett + 'g'
                    guesses += guessLett
                    print("Buchstabe g gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('g')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('g')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_h:
#Buchstabe h
                    guessLett = guessLett + 'h'
                    guesses += guessLett
                    print("Buchstabe h gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('h')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('h')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_i:
#Buchstabe i
                    guessLett = guessLett + 'i'
                    guesses += guessLett
                    print("Buchstabe i gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('i')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('i')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_j:
#Buchstabe j
                    guessLett = guessLett + 'j'
                    guesses += guessLett
                    print("Buchstabe j gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('j')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        #gamePlay = Falsch
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('j')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        #gamePlay = Falsch
                        endGame()
                            
                if event.key == pygame.K_k:
#Buchstabe k
                    guessLett = guessLett + 'k'
                    guesses += guessLett
                    print("Buchstabe k gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('k')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('k')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_l:
#Buchstabe l
                    guessLett = guessLett + 'l'
                    guesses += guessLett
                    print("Buchstabe l gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('l')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('l')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_m:
#Buchstabe m
                    guessLett = guessLett + 'm'
                    guesses += guessLett
                    print("Buchstabe m gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('m')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('m')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_n:
#Buchstabe n
                    guessLett = guessLett + 'n'
                    guesses += guessLett
                    print("Buchstabe n gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('n')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        #gamePlay = Falsch
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('n')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        #gamePlay = Falsch
                        endGame()
                            
                if event.key == pygame.K_o:
#Buchstabe o
                    guessLett = guessLett + 'o'
                    guesses += guessLett
                    print("Buchstabe o geraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('o')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('o')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_p:
#Buchstabe p
                    guessLett = guessLett + 'p'
                    guesses += guessLett
                    print("Buchstabe p gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('p')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('p')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_q:
#Buchstabe q
                    guessLett = guessLett + 'q'
                    guesses += guessLett
                    print("Buchstabe q gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('a')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('q')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_r:
#Buchstabe r
                    guessLett = guessLett + 'r'
                    guesses += guessLett
                    print("Buchstabe r gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('r')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('r')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_s:
#Buchstabe s
                    guessLett = guessLett + 's'
                    guesses += guessLett
                    print("Buchstabe s gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('s')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('s')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_t:
#Buchstabe t
                    guessLett = guessLett + 't'
                    guesses += guessLett
                    print("Buchstabe t gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('t')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('t')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_u:
#Buchstabe u
                    guessLett = guessLett + 'u'
                    guesses += guessLett
                    print("Buchstabe u gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1
                    if guessLett in pick:
                        placeLetter('u')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('u')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_v:
#Buchstabe v
                    guessLett = guessLett + 'v'
                    guesses += guessLett
                    print("Buchstabe v gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('v')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('v')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_w:
#Buchstabe w
                    guessLett = guessLett + 'w'
                    guesses += guessLett
                    print("Buchstabe w gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('w')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('w')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                            
                if event.key == pygame.K_x:
#Buchstabe x
                    guessLett = guessLett + 'x'
                    guesses += guessLett
                    print("Buchstabe x gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1
                    if guessLett in pick:
                        placeLetter('x')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('x')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                    
                if event.key == pygame.K_y:
#Buchstabe y
                    guessLett = guessLett + 'y'
                    guesses += guessLett
                    print("Buchstabe y geraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('y')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('y')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()
                    
                if event.key == pygame.K_z:
#Buchstabe z
                    guessLett = guessLett + 'z'
                    guesses += guessLett
                    print("Buchstabe z gerraten")
                    print("")
                    for char in pick:
                        if char in guesses:
                            print(char)
                        else:
                            print("_")
                            failed += 1

                    if guessLett in pick:
                        placeLetter('z')
            
                    if failed == 0:
                        print("Du hast das Wort erraten")
                        print(pick)
                        endGame()

                    if guessLett not in pick:
                        textBoxSpace += 40
                        textBoxNumber += 1
                        chances = chances - 1
                        print("")
                        print(textBoxNumber)
                        print("")
                        print("Dieser Buchstabe ist nicht im Wort")
                        textBoxLetter('z')

                    if chances == 0:
                        print("Sorry du hast verloren")
                        print("Das Wort war",pick)
                        endGame()

                    

        pygame.display.update()
        clock.tick(fps)

    pygame.display.update()
    clock.tick(fps)

def Animals():
    animal = ['kuh','hund','katze','schwein','zebra','vogel','giraffe','löwe','tiger','pinguin','hamster','fuchs','panda','bear','gepard','strauss','erdmännchen','wal','hai','ross','affe','oktopus','katze','kangoru','huhn','fisch','hase','schaf']
    print("Tier")
    title = "Tiere"
    hangmanGame(animal,title)

def Vehicles():
    vehicle = ['auto','bus','zug','kutsche','flugzeug','schiff','jet','boot','lastwagen','traktor','bike','motorrad','tram','van','ambulanz','feuerwehr','polizei','taxi','caravan','coach','skateboard','scooter','schlitten','tanklastwagen','wagen','raumschiff']
    print("fahrzeuge")
    title = "Fahrzeuge"
    hangmanGame(vehicle,title)
    
def Foods():
    food = ['apfel','banane','orange','pfirsich','pizza','donut','chips','sandwich','cookie','gurke','karotte','mais','glace','pfannkuchen','brot','kartoffel','tomate','nuss','yogurt','pasta','reis','käse','suppe','fisch','ei','fleisch','schinken','salat']
    print("essen")
    title = "Essen"
    hangmanGame(food,title)
    
def Sports():
    sport = ['rugby','fussball','volleyball','basketball','schwimmen','hockey','curling','rennen','golf','tennis','badmington','bogenschiessen','klettern','bowling','tanzen','gym','skating','baseball','rounders','boxen','reiten','kanufahren','velofahren','fechten','karate','schiessen','kricket']
    print("sport")
    title = "Sportarten"
    hangmanGame(sport,title)

    
if __name__ == '__main__':
    main()

